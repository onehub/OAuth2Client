...
{
  [self authenticateWithUsername:username password:password success:nil failure:nil];
}
...
- (void)oauthConnection:(NXOAuth2Connection *)connection didFinishWithData:(NSData *)data;
{
  if (connection == authConnection) {
    ...
    if (self.authSuccessBlock) {
      self.authSuccessBlock(newToken);
    }
    
    self.authSuccessBlock = nil;
    self.authFailureBlock = nil;
