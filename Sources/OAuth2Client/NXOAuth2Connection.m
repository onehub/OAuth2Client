...
- (void)dealloc;
{
  if (sendConnectionDidEndNotification) {
    [[NSNotificationCenter defaultCenter] postNotificationName:NXOAuth2ConnectionDidEndNotification
                                                        object:self];
  }
  sendConnectionDidEndNotification = NO;
...
- (NSURLConnection *)createConnection;
{
  ...
  NSURLConnection *aConnection = [[NSURLConnection alloc] initWithRequest:startRequest
                                                                 delegate:self
                                                         startImmediately:NO];    // don't start yet
  [aConnection scheduleInRunLoop:[NSRunLoop currentRunLoop]
                         forMode:NSDefaultRunLoopMode];    // let's first schedule it in the current runloop. (see http://github.com/soundcloud/cocoa-api-wrapper/issues#issue/2 )
  [aConnection start];    // now start

  if (!sendConnectionDidEndNotification) {
    [[NSNotificationCenter defaultCenter] postNotificationName:NXOAuth2ConnectionDidStartNotification
                                                        object:self];
  }
  ...
  if ([httpMethod caseInsensitiveCompare:@"POST"] != NSOrderedSame
    ...
    if (!contentType || [contentType isEqualToString:@"multipart/form-data"]) {
      ...
      contentType = [NSString stringWithFormat:@"multipart/form-data; boundary=%@",
                                               [(NXOAuth2PostBodyStream *) postBodyStream boundary]];
...
- (void)stream:(NXOAuth2PostBodyStream *)stream
  didSendBytes:(unsigned long long)deliveredBytes
       ofTotal:(unsigned long long)totalBytes;
{
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection;
{
  ...
  if (self.statusCode < 400) {
    ...
    NSString     *localizedError = [NSString stringWithFormat:NSLocalizedString(@"HTTP Error: %d", @"NXOAuth2HTTPErrorDomain description"),
                                                              self.statusCode];
            ...
            redirectResponse:(NSURLResponse *)aRedirectResponse;
{
  ...
  if (hostChanged || (schemeChanged && !schemeChangedToHTTPS)) {
    ...
    [mutableRequest setValue:[NSString stringWithFormat:@"%@ %@",
                                                        client.accessToken.tokenType,
